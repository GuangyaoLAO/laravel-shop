...
class UsersController extends Controller
{
    ...
    /*
     * 其中 edit() 和 create() 用于编辑和创建用户，由于我们不会在管理后台去新增和编辑用户，
     * 所以可以把这两个方法删除，同时可以删除掉与之相关的 form() 方法。
        show() 方法用来展示用户详情页，通过调用 detail() 方法来决定要展示哪些字段，Laravel-Admin
        会通过读取数据库自动把所有的字段都列出来，由于我们的用户表没有太多多余的字段，在列表页就可以直
        接展示，因此可以把 show() 和 detail() 方法也删掉。
        index() 方法用来展示用户列表，通过调用 grid() 方法来决定要展示哪些列，以及各个字段对应的名称，
